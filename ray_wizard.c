...
void	ray_caster(t_scene *scene)
{
	...
	while (x < WIN_WIDTH)
	{
		...
		int	linestart = scene->player->central_angle - scene->player->crouch - scene->player->vision_rays[x]->wall_height / 2 + WIN_HEIGHT / 2;
		int truestart = linestart;
		if (linestart < 0)
			linestart = 0;
		int lineend = scene->player->central_angle - scene->player->crouch + scene->player->vision_rays[x]->wall_height / 2 + WIN_HEIGHT / 2;
		int trueend = lineend;
